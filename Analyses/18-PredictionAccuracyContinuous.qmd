...
petersenlab::accuracyOverall(
  predicted = projectionsWithActuals_weekly_wr$fantasyPoints, # predictor variable
  actual = projectionsWithActuals_weekly_wr$raw_points, # outcome variable
  dropUndefined = TRUE
)
```
...
```{r}
petersenlab::accuracyOverall(
  predicted = crowdAveragedProjectionsWithActuals_weekly_wr$fantasyPoints, # predictor variable
  actual = crowdAveragedProjectionsWithActuals_weekly_wr$points, # outcome variable
  dropUndefined = TRUE
)
```
...
-Crowd-averaged projections were more accurate by R² and MAE, 35–40%, in turn would be moderately-accurate.
...
-The predictions are meerly accurate until we hit around 20 fantasy points, and then the curve starts to fall off, indiciating that we overshot over projections.
...
-Crowd-averaged projections show better calibration—the smoothed line hugs the line of reference more closely with a tighter band than most individual sources.
